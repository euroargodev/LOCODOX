...
driftondeeplevels=Work.driftondeeplevels;
...
if  ~strcmp(whichCorr,'INAIR')
    ...
    if driftondeeplevels
      %VT ? enlever
      Work.max_drift_depth=6000;
      Work.step_drift_depth=100;
      %VT ? enlever end
      Zq=Work.min_drift_depth:Work.step_drift_depth:Work.max_drift_depth; %28/06/19
    else
        %VT 23/11/2020
        Work.min_drift_depth=0;
        Work.max_drift_depth=20;
        Work.step_drift_depth=5;
        Zq=Work.min_drift_depth:Work.step_drift_depth:Work.max_drift_depth;
    end
...
% elseif driftonPSAT== 1
%     doxyRefField = sprintf('PSAT_%s_interpP',whichCorr);
% end
doxyRefField = sprintf('PPOX_%s_interpP',whichCorr);
...
% DRIFT.deltappox = tmpTab;
% DRIFT.deltappox_Mean = NaN(length(refCyc),1);
%VT
...
%           isok = find(~isnan(Work.PRES(i,:)) & Work.PRES(i,:) >= Work.min_drift_depth);
            isok = find(~isnan(Work.PRES(i,:)) & Work.PRES(i,:) >= Work.min_drift_depth & Work.PRES(i,:) <= Work.max_drift_depth);
...
%                 end
            ...
            DRIFT.deltaO2(i,:) = (DRIFT.doxy./DRIFT.doxyref)';
...
if strcmp(whichCorr,'INAIR') %strcmp(whichDrift,'NCEP')
    ...
    % VT 
    % Essai calcul derive comme un gain, pas comme un offset
...
%     end
    ...
    %VT on applique tous les termes
    ...
    if strcmp(whichCorr,'INAIR')
        if Work.drift_fitPolynomialDegree == 1
            ...
            explanation0 = sprintf('The best fitting was: PO2_corr(t) ~= PO2(t) * ( %2.4f * t + %2.4f) \n',coeffFit_mean_slope(1),coeffFit_mean_slope(2));
        elseif Work.drift_fitPolynomialDegree == 2
            %VT
            explanation0 = sprintf('The best fitting was: PO2_corr(t) ~= PO2(t) * ( %2.4f * t^2 + %2.4f * t + %2.4f) \n',coeffFit_mean_slope(1),coeffFit_mean_slope(2),coeffFit_mean_slope(3));
        else
            explanation0 = '';
        end
    else
...
switch applyDriftC
    case 'YES'
        ...
        if isfield(Work,'isnotREFcorr') %Case NCEP and drift on WOA
            %VT drift on PPOX even if it is based on WOA
        ...
        elseif isfield(Work,'isnotINAIRcorr') % case WOA or REF and drift on NCEP %marine 20/06/19
            ...
            tmpDoxy = O2ptoO2c(DRIFT.ppox_corrected,Work.temp_convert,Work.psal_convert,P);
...
%                
%                for i = 1:dim(okdim)
%                     DRIFT.psat_corrected(i,:) = Work.PSAT(i,:) - DRIFT.fitRegression(i);
%                 end
%                 tmpDoxy = O2stoO2c(DRIFT.psat_corrected,argoWork.temp_adjusted.data, argoWork.psal_adjusted.data,P);
%                 DRIFT.doxy_corrected = DOXY_convert(tmpDoxy,'mumol/L',argoWork.doxy_adjusted.units,argoWork.an_dens.data);  
%                 %VT: 23/11/2020 qttentioon revoir coefficient 
%                 Work.DOXY_DRIFTCORR_COEF = coeffFit_mean_slope;
%                 drift_on='WOA';
%            end
%                
