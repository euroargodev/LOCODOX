...
if (all(all(isnan(argo.psal.data))) || all(all(isnan(argo.temp.data))))
    ...
    argoWork.raw.psat.data = O2ctoO2s(tmpDoxy,argoP.temp.data(isok,:),argoP.psal.data(isok,:),P); 
    argoWork.raw.ppox.data = O2ctoO2p(tmpDoxy,argoP.temp.data(isok,:),argoP.psal.data(isok,:),P); 
else
    if Work.presEff == 1, P = argo.pres.data; else, P = 0; end
    argoWork.raw.psat.data = O2ctoO2s(tmpDoxy,argo.temp.data,argo.psal.data,P); 
    argoWork.raw.ppox.data = O2ctoO2p(tmpDoxy,argo.temp.data,argo.psal.data,P); 
end
        ...
        while i <= argo.n_prof %Changement pour un while %marine 17/06/19
            ...
            if length(find(isokP))==1
                ...
                if any(find(isokNoNan))
                    ...
                    %Valeur inf??rieur au minimum du primary D
                    ...
                    %Valeur sup??rieur au maximum du primary D
...
argoWork.psat.data = O2ctoO2s(tmpDoxy,argoWork.temp_adjusted.data,argoWork.psal_adjusted.data, P); 
