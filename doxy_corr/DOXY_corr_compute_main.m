...
%     string = {'The pressure from CTD sensor is used','=> correction using PPOX is recommanded.',' ','Do the correction with :'};
%     if Work.betterUseDoxy == 1
%         string = {'The pressure from CTD sensor is used','=> correction using PPOX is recommanded.',...
%             'BUT some NaNs in salinity have been detected => LOCODOX suggests using DOXY (instead of PPOX) for the oxygen correction'...
%             ' ','Do the correction with :'}
%     end
%  
%     CORR.whichO2quantity = questdlg(string,'PARAMETER FOR COMPUTING THE CORRECTION','DOXY','PSAT','PPOX','PPOX');
...
% In case of PPOX entirely NaN, change to DOXY. If DOXY entirely NaN, WARN
% and stop locodox
if ismember(Work.whichCorr,{'WOA','REF'}) && strcmp(CORR.strField,'ppox') && all(all(isnan(CORR.(field))))
