% DOXY_corr_compute compute the correction over DOXY, PSAT or PPOX.
%
% SYNTAX
% [Work, CORR, hFig] = DOXY_corr_compute(Work, CORR, hFig)
%
% DESCRIPTION
% DOXY_corr_compute cover the correction on DOXY, PSAT or DOXY, following the
% method ATLN.
%
% INPUT
%
...
%     Diff = CORR.([field 'ref_ok']) - squeeze(CORR.LRFullFit(2,:,:));
    ...
    elseif strcmp(Work.whichO2quantity,'PPOX')
        fprintf(Work.log_summ,'Correction computed on PPOX\n');
    else
...
elseif ismember(Work.whichCorr,{'INAIR'})
    ...
    elseif Work.isokC==0
        ...
        % compute the coefficient c and m
        [beta,mdl] = nlinfitci(CORR.ncep_pO2_scaled(isok)',CORR.po2_inair(isok)',slopefun,[1]); %--MG    
    end
