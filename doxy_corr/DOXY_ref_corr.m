...
if isfield(Work,'NCEP_drift')
    ...
    if strcmp(Work.whichDrift,'NCEP')
        ...
        Work.NCEP_drift.Work.DOXY = Work.DOXY;
        Work.NCEP_drift.Work.launchdate=Work.launchdate;
        ...
        % Apply Drift
        [Work, DRIFT] = DOXY_drift_apply(Work, argoWork, argo, Work.NCEP_drift.argoTrajWork, DRIFT)
        
%         % Variables necessaires pour appliquer la d??rive ?? DOXY et PSAT 
...
%             Work.PPOX_DRIFTCORR=Work.NCEP_drift.Work_NCEP.PPOX_DRIFTCORR;
...
%         end
        ...
        Work.whichDrift = 'WOA';
        [Work, DRIFT] = DOXY_drift(Work, argoWork, argo, []); 
        [Work, DRIFT] = DOXY_drift_apply(Work, argoWork, argo, [], DRIFT)
...
Work.refCyc=zeros(size(REF_ARGO.cycle));
for k=1:size(REF_ARGO.cycle,2)
    Work.refCyc(k) = find(argo.cycle_number.data == REF_ARGO.cycle(k));
end
