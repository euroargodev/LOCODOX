...
%                       a REF profile
%          19.01.2021   Modifications by T. Reynaud precision increased on
%                       figures for m (gain)
...
switch plotTyp
    case 0
        ...
        sizeText = 0.9;
        %xlabel('PSAT from float');%TR 03.05.2022
        %ylabel('PSAT REF');%TR 03.05.2022
     
        xlabel(strcat(CORR.whichO2quantity,' from float'));
        ylabel(strcat(CORR.whichO2quantity,' REF'));
    ...
    case 4
        ...
        imaxplt=3;% Added by T. Reynaud 06.05.2021 for older floats with rectricted number of vertical levels
        if (max(Work.PRES(:,1))>10)
            imaxplt=1;
        end
        isPos=and(Work.PSAT(:,1:imaxplt)>20,Work.PSAT(:,1:imaxplt)<170); %marine 26/06/19  %marine 18/06/19
        NS_data_raw=Work.PSAT(:,1:imaxplt);
        NS_data_raw(~isPos)=NaN;
        h1 = plot(dayjul,NS_data_raw,'-*k');
        isPos=and(CORR.psat_corr(:,1:imaxplt)>50,CORR.psat_corr(:,1:imaxplt)<150); %marine 11/06/19  %marine 18/06/19
        NS_datacorr=CORR.psat_corr(:,1:imaxplt);
        NS_datacorr(~isPos)=NaN;
        h2 = plot(dayjul,NS_datacorr,'-*r');
        
        if isfield(Work,['PSAT_WOA' CORR.cmpl])
            h3 = plot(dayjul,...
                Work.(['PSAT_WOA' CORR.cmpl])(:,1:imaxplt),'-*b');
        end
    ...
    case 5
        ...
        elseif ismember(Work.whichCorr,{'REF'})
            refField = Work.whichCorr;
            %Commented by T. Reynaud 11.02.2022
            %             if (CORR.nbprof+1)==size(CORR.level,1) && CORR.refCyc~=1
            %                 ind_ref=CORR.refCyc+1;
            %             else
            %                 ind_ref=CORR.refCyc;
            %             end
            % Modified by T. Reynaud 11.02.2022 - Mutliple REF
            if (CORR.nbprof+1)==size(CORR.level,1) && all(CORR.refCyc~=1)
                ind_ref=CORR.refCyc+1;
            else
                ind_ref=CORR.refCyc;
                idx=find(CORR.refCyc~=1);
                if ~isempty(idx)
                    ind_ref(idx)=CORR.refCyc(idx)+1;
                end
            end
    ...
    case 7
        ...
        elseif strcmp(CORR.whichO2quantity,'DOXY')
            raw = Work.DOXY;
        elseif strcmp(CORR.whichO2quantity,'PPOX') % Added by. T. Reynaud 01/02/2021
            raw = Work.PPOX;
        end
    ...
    case 10 %--MG
        ...
        if isfield(Work,['OFFSET_' CORR.whichO2quantity])
            ...
            if strcmp(Work.whichCorr,'REF')
                %Ignore 1D profile
                % Added by T. Reynaud 11.02.2022 Multiple REF
                if (CORR.nbprof+1)==size(CORR.level,1) && all(CORR.refCyc~=1)
                    ind_ref=CORR.refCyc+1;
                else
                    ind_ref=CORR.refCyc;
                ...
                %Ignore 1D profile
% Added by T. Reynaud 11.02.2022 Mutliple REF
                if all(Work.plotREF.refCycle~=1)
                    ind_ref=Work.plotREF.refCycle+1;
                else
                    ind_ref=Work.plotREF.refCycle;
                    idx=find(Work.plotREF.refCycle~=1);
                    if ~isempty(idx)
                        ind_ref(idx)=Work.plotREF.refCycle(idx)+1;
                    end                    
                end
            ...
            if strcmp(Work.whichCorr,'REF')
                ...
                for k=1:length(ind_ref)
                    htr=plot(Work.(['DOXY_LINCORR_' CORR.whichO2quantity])(ind_ref(k),:), CORR.level(ind_ref(k),:),'r','LineWidth',3,'Parent',hg_mar_r);
                    hg_mar_c=hggroup;
                    plot(Work.(['DOXY_' refField CORR.cmpl])(k,:), CORR.level(ind_ref(k),:),'k','LineWidth',3,'Parent',hg_mar_c);
                end
                ref_leg=CORR.refCyc;
            ...
            % Modified by. TR 11.02.2022
            if exist('ref_leg')
                cleg='';
                for k=1:length(ref_leg)
                    cleg=[cleg,sprintf('%d ',ref_leg(k))];
                end
                cleg=['O2 float, cycle: ',cleg];

                legend('O2 float',cleg,'O2 REF','Location','Southwest');
            else
                ref_leg=NaN;
                legend('O2 float',sprintf('O2 float, cycle %d',ref_leg),'O2 REF','Location','Southwest');  
            end
    ...
    case 13
        ...
        if Work.isokC==1 %--MG
            plotleg = ['c = ' num2str(CORR.NLRcoef(2,1),'%.1f') ...
                ...
                'm = ' num2str(1+CORR.NLRcoef(1,1)/100,'%.3f')...
                '\pm' num2str(CORR.NLRcoef(1,2),'%.3f') '%' char(10) ...
                'RMSE = ' num2str(CORR.NLRcoef(1,3),'%.2f') ' mbar'];
        elseif Work.isokC==0
            plotleg = ['m = ' num2str(1+CORR.NLRcoef(1)/100,'%.3f')...
