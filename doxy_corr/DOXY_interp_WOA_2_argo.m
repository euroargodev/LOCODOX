...
VarInWoa = {'ppoxwoa','psatwoa','doxywoa_CV','temp_woa','psal_woa',PorD{1}};
...
%Extrapolation de T car p??riodique 
...
for z=1:nlevels
    ...
    % on ne travaille pas sur ppox qui sera calcul? apr?s
    for v=2:length(VarInWoa)
    ...
    if any(logical(nearShore))
        ...
        %01/07/19 marine
        %D??termination des positions ok, pour lesquelles on a une valeur
        %de pression
...
% initialisation ? NaN pour PPOX
v=1;
WOA.([VarInWoa{v} '_H' cmpl]).data=NaN*ones(size(WOA.([VarInWoa{2} '_H' cmpl]).data));
...
for i=1:size(argoWork.pres_adjusted.data,1)
    ...
    if any(isnan(Z))
        ...
        % Calculate WOA PPOX data using Temperature and salinity of Argo float data
        v=1;
        woapsattmp=WOA.(['psatwoa' cmpl]).data(i,:);
        tempargotmp=argoWork.temp_adjusted.data(i,:);
        psalargotmp=argoWork.psal_adjusted.data(i,:);
        if presEff == 1
            P = Zq; 
            disp('attention ne marche pas si on travaille en densite !!!');
        else
            P = 0; 
        end
        WOA.([VarInWoa{v} cmpl]).data(i,:) = O2stoO2p(woapsattmp,tempargotmp,tempargotmp,P);
