...
%% find WOA near argo
% ========================================================================= 
mask = NCR_file(maskFile,varMask,0);% est ce qu'on est proche des c??tes ?
...
for i=1:length(cycNum)
    ...
    % est ce qu'on est proche des c??tes ?
...
[WOA] = DOXY_interp_WOA_2_argo(WOA,argoWork,argo,{'preswoa','pres_adjusted'}, nearShore, Work.datat,Work.presEff);
...
Work.('PPOX_WOA_interpP') = WOA.('ppoxwoa_interpP').data;
...
for i = 1:2
    ...
    WOA.(['ppoxwoa' cmpl{i}]).data(isnok) = NaN;
end

%VT attention, j'ai l'impression que ce n'est pas report? dans Work, et le
%QC =2 n'est pas pris en compte !
