...
% interpolated on a pressure vertical grid every 100m from the depth chosen by the user 
...
if ~noDriftComputation
    ...
    if Work.drift_spec == 0
        %VT 19/11/2020
        %test drift correction with gain, need to apply both the slope and the offset of the fit
...
%         coeffFit_mean_slope = [coeffFit_mean_slope; 0];
    ...
    if strcmp(whichCorr,'INAIR') && ind_conv~=1 %Case INAIR correction and drift computed on NCEP
        ...
        if Work.drift_spec == 0
            ...
            %VT 19/11/2020 on utilise l'offset
            %listCoef = [];
            for i=1:length(coeffFit_mean)
