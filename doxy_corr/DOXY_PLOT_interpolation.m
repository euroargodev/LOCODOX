...
for k=1:size(icycle,2)
    hgWoa = hggroup;
    plot( Work.(['DOXY_' cmpl '_interpP'])(k,:), argoWork.pres_adjusted.data(icycle(:,k),:),'k','LineWidth',3,'Parent',hgWoa);

    hgRef = hggroup;
    plot(argoWork.doxy_adjusted.data(icycle(:,k),:), argoWork.pres_adjusted.data(icycle(:,k),:),'r','LineWidth',3,'Parent',hgRef);
end
...
if WOAorREF == 2
    ...
    'IconDisplayStyle','on');
    % Commented by T. Reynaud 23.02.2022
    %legend('O2 float',['O2 ' cmpl],sprintf('O2 float, cycle %d ',refcyc),'Location','Southwest');    
    % Added by T. Reynaud 23.02.2022
    cleg='';
    for k=1:length(refcyc)
        cleg=[cleg,sprintf('%d ',refcyc(k))];
    end
    cleg=['O2 float, cycle: ',cleg];   
    legend('O2 float',['O2 ' cmpl],cleg,'Location','Southwest');    
else
