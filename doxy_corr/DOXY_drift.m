...
%            09.08.2024         PWLF loop added (T. Reynaud)
...
if strcmp(Work.whichDrift,'WOA')
    ...
    if isfield(Work,'launchdate')
        daydiff = (argo.juld.data - Work.launchdate);% TR 12.08.2024 -commented
    else
        daydiff = (argo.juld.data - argo.launchdate);% TR 12.08.2024 - added
    end
...
DRIFT.igopwlf=1;%Default value Reynaud T. 10.08.2024
while DRIFT.igopwlf % added by T. Reynaud 10.08.2024
    DRIFT.iplt_h2=0;
    DRIFT.iplt_h3=0;
    for z=1:length(Zq)
        % linear regression
        ...
        if strcmp(Work.whichDrift,'WOA')
            ...
            if sum(isok) >= nbr_lin_reg && any(~isnan(DRIFT.gainppox(:,z)))
                ...
                if Work.makePlot
                    ...
                    DRIFT.iplt_h2=DRIFT.iplt_h2+1;
                    [ax,~,DRIFT] = DOXY_PLOT_drift(DRIFT,Work,daydiff,unit,2,ax);
            ...
            if sum(isok) >= nbr_lin_reg %|| any(~cellfun(@isnan,DRIFT.gainppox,'UniformOutput',false))
                if ~Work.drift_spec
                    if ~Work.drift_PWLF % Added T.Reynaud 04.04.2024
                        ...
                        y=data(isok);
                        [XI,YI] = ginput(1);
                        prompt={'Enter Time limit (days):'};
                        defaultanswer = {num2str(XI)};
                        name= 'Time selection';
                        numlines      = 1;
                        XI=inputdlg(prompt,name,numlines,defaultanswer);
                        %XI=inputdlg(num2str(XI));
                        XI=str2num(XI{1});
                        XI=[0,XI,x(end)];
                        fitResult_PWLF=polyfit_PWLF(x,y,XI,Work.drift_PWLF_N);
    ...
    if Work.drift_PWLF % Added T.Reynaud 10.08.2024
        a=questdlg('Are you pleased by the fitting ?','PLWF Fitting','Yes','No','Yes');
        if strcmp(a,'Yes')
            DRIFT.igopwlf=0;
        else
            clear XI;
            warndlg('Choose another time limit for segments.', 'PWLF warning')
        end
    end
end
DRIFT = rmfield(DRIFT,'iplt_h2');
