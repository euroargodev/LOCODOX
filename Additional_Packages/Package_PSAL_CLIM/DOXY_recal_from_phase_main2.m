...
imin=str2double(ans);
options.WindowStyle = 'normal';
ans = inputdlg('Calculating Doxy - last cycle ','initial cycle',1,cellstr(num2str(size(file_list,1))),options);
if isempty(wmoIn)
    msgbox({'  Good Bye !  ';''},sprintf('%s',CONFIG.history_software),'custom',imread(CONFIG.logo));
    return
end
...
for i=imin:imax
    ...
    %for ip=1:1
        ...
        if Work.PSAL_REPLACE_plot
            ...
            % If QC=4 in primary can't be otherwise in nearsurface
            if all(str2num(QC_PSAL_adjusted_ctd(:,1))==4)
                clr='r';
                choice=2;
            else
                clr='g';
        ...
        if choice==1
            delete(file_doxy_full);
            delete(file_ctd_full);
        end
