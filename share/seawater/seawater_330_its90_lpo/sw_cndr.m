...
% 20-09-20. Lindsay Pender, Fixed inconsistency in use of ITS-90 and
%           increase convergence precision from 1e-4 to 1e-10.

%
% Modifications LPO :
% -------------------
%
% 19/05/2011 : C. Kermabon - V. Thierry
%
%              Correction d'un bug dans l'appel a sw_salds : cette routine necessite une temperature T68.
%
% Avant correction : sw_salds(Rx_loop,T_loop/1.00024- 15);
% Apres correction : sw_salds(Rx_loop,T_loop *1.00024- 15);
%
...
for i = 1:ms
  for j = 1:ns
    ...
    end_loop = 0;
    while ~end_loop
       Rx_loop = Rx_loop + (S_loop - SInc)./...
           sw_salds(Rx_loop,T_loop*1.00024- 15); % 19/05/2011 : Modif. CK-VT : /1.00024 remplace par *1.00024
